...
// VERSÃO COM API - BACKEND NODE.JS + SQLITE
        ...
        function validateSimulation(sim) {
            return Boolean(sim && typeof sim.cliente === 'string' && sim.cliente.trim() && sim.data_aplicacao);
        }
        ...
        async function loadHistory(forceRefresh = false) {
            try {
                ...
                const now = Date.now();
                if (!forceRefresh && cachedHistory && (now - cacheTime < HISTORY_CACHE_DURATION)) {
        ...
        function getProdutoResumo(item) {
            if (item.produtos_nomes) {
                return item.produtos_nomes.split('|').filter(Boolean).join(', ');
            }
            if (Array.isArray(item.produtos)) {
                return item.produtos.map(p => p.produto_nome || p.nome || '').filter(Boolean).join(', ');
            }
            return '';
        }
        ...
        auth.onAuthStateChanged(async (user) => {
            if (user) {
                ...
                try {
                    ...
                    isUserAdmin = user.email === ADMIN_EMAIL || userData?.isAdmin || false;
