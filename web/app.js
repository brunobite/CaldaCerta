...
// VERSÃO COM API - BACKEND NODE.JS + SQLITE
        ...
        function renderHistoryList(data) {
            ...
            list.innerHTML = data.map(item => {
                ...
                const produtosDetalhe = Array.isArray(item.produtos)
                    ? item.produtos.map(p => {
                        const nome = p.produto_nome || p.nome || 'Produto';
                        const dose = p.dose ? ` - ${p.dose} L/Kg/ha` : '';
                        return `<li>${nome}${dose}</li>`;
                    }).join('')
                    : '';
                ...
                return `
                    <div class="card card-medium history-card" onclick="viewSimulation('${item.id}', '${item.uid || ''}')">
                        ...
                        ${isUserAdmin && produtosDetalhe ? `
                            <div class="mt-2 text-xs text-slate-600">
                                <strong>Mix de calda:</strong>
                                <ul class="mt-1">${produtosDetalhe}</ul>
                            </div>
                        ` : ''}
                    </div>
                `;
